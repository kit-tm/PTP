...
public class PTPDuplexExample {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      while ((listener1.sent() < max || listener2.sent() < max)
          && System.currentTimeMillis() - waitStart <= 120 * 1000) {
        try {
          // Sleep.
          Thread.sleep(5 * 1000);
        } catch (InterruptedException e) {
      ...
      System.out
          .println("\t RTT client1          : " + listener1.duration() / listener1.sent() + " ms.");
      System.out
          .println("\t RTT client2          : " + listener2.duration() / listener2.sent() + " ms.");
    } catch (IOException e) {
      e.printStackTrace();
