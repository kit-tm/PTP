...
/**
 * A thread pool with limited number of workers. Offers access to the worker with least current
 * estimated load.
 *
...
public class ThreadPool<Item, PoolWorker extends Worker<Item>> {
  ...
  /**
   * Returns the worker with least current estimated load. Performs a linear scan to select that
   * worker.
   *
