...
public abstract class Ed25519Mod
{
...
//        assert carry == 0;
    ...
    {
        // TOR:   crypto_hash_sha512_2(nonce, sk+32, 32, m, mlen);
        dom2(d, phflag, ctx);
        ...
        // TOR:  ge_scalarmult_base(&R,nonce);
        scalarMultBaseEncoded(r, R, 0);

        // TOR:  crypto_hash_sha512_3(hram, sig, 32, pk, 32, m, mlen);
        dom2(d, phflag, ctx);
        ...
        // TOR:  sc_muladd(sig + 32,hram,sk,nonce); // sig=hram*sk+nonce   // I think this is: S = k * s + r
        byte[] S = calculateS(r, k, s); // S = k * s + r

        // TOR:  ge_p3_tobytes(sig,&R);
        ...
        byte[] m, int mOff, int mLen, byte[] sig, int sigOff)
    {
        ...
        //d.update(sk, skOff, SECRET_KEY_SIZE);
        //d.doFinal(h, 0);
        System.arraycopy(sk, skOff, h, 0, d.getDigestSize());

        //byte[] s = new byte[SCALAR_BYTES];
        //pruneScalar(h, SCALAR_BYTES, s);
        byte[] s = new byte[64];
        System.arraycopy(sk, skOff, s, 0, 64);
